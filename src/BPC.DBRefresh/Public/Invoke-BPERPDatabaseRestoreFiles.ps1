function Invoke-BPERPDatabaseRestoreFiles {
  ...
  if ($BackupFiles.ASPNET) {
    Write-BPlusLog -Message "Restoring ASPNET database from: $($BackupFiles.ASPNET)" -LogPath $script:LogPath
    ...
    try {
      ...
      Write-BPlusLog -Message "ASPNET database restored successfully" -LogPath $script:LogPath
    }
    catch {
      Write-BPlusLog -Message "Error restoring ASPNET database: $_" -Level Error -LogPath $script:LogPath
      throw
    }
  }
  
  # Restore SYSCAT database
  Write-BPlusLog -Message "Restoring SYSCAT database from: $($BackupFiles.SYSCAT)" -LogPath $script:LogPath
  ...
  try {
    ...
    Restore-DbaDatabase @restoreParams
    Write-BPlusLog -Message "SYSCAT database restored successfully" -LogPath $script:LogPath
  }
  catch {
    Write-BPlusLog -Message "Error restoring SYSCAT database: $_" -Level Error -LogPath $script:LogPath
    throw
  }
  
  # Restore IFAS database
  Write-BPlusLog -Message "Restoring IFAS database from: $($BackupFiles.IFAS)" -LogPath $script:LogPath
  ...
  try {
    ...
    Restore-DbaDatabase @restoreParams
    Write-BPlusLog -Message "IFAS database restored successfully" -LogPath $script:LogPath
  }
  catch {
    Write-BPlusLog -Message "Error restoring IFAS database: $_" -Level Error -LogPath $script:LogPath
    throw
  }
  
  Write-BPlusLog -Message "All database restores completed successfully" -LogPath $script:LogPath
}