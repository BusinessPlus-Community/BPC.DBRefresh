function Invoke-BPERPDatabaseRestore {
  ...
  #>
    ...
    begin {
        ...
        # Set up logging
        $script:LogPath = Join-Path $PSScriptRoot -ChildPath ".." | Join-Path -ChildPath ".." | Join-Path -ChildPath ".." | Join-Path -ChildPath ".." | Join-Path -ChildPath "BPC.DBRefresh.log"
        Start-Log -LogPath $script:LogPath -LogName "BPC.DBRefresh" -ScriptVersion $script:ModuleVersion

        Write-BPERPLog -Message "Starting BusinessPlus Database Restore for environment: $BPEnvironment" -LogPath $script:LogPath
        ...
        try {
            ...
            Write-BPERPLog -Message "Failed to load required modules: $_" -Level Error -LogPath $script:LogPath
            throw
        }

        # Set default config path if not provided
        if (-not $ConfigPath) {
            $ConfigPath = Join-Path $PSScriptRoot -ChildPath ".." | Join-Path -ChildPath ".." | Join-Path -ChildPath ".." | Join-Path -ChildPath ".." | Join-Path -ChildPath "config" | Join-Path -ChildPath "BPC.DBRefresh.ini"
        }
    ...
    process {
        try {
            # Load configuration
            Write-BPERPLog -Message "Loading configuration from: $ConfigPath" -LogPath $script:LogPath
            $config = Get-BPERPEnvironmentConfig -Environment $BPEnvironment -ConfigPath $ConfigPath
            ...
            if (-not $SkipConfirmation) {
                $confirmed = Show-BPERPConfiguration -Config $config -BackupFiles $backupFiles -TestingMode $TestingMode -RestoreDashboards $RestoreDashboards
                if (-not $confirmed) {
                    Write-BPERPLog -Message "Restore operation cancelled by user" -Level Warning -LogPath $script:LogPath
                    return
                }
            }

            # Stop BusinessPlus services
            Write-BPERPLog -Message "Stopping BusinessPlus services..." -LogPath $script:LogPath
            Stop-BPERPServices -Config $config

            # Get existing database settings
            Write-BPERPLog -Message "Backing up existing database connection settings..." -LogPath $script:LogPath
            $existingSettings = Get-BPERPDatabaseSettings -Config $config

            # Restore databases
            Write-BPERPLog -Message "Starting database restore operations..." -LogPath $script:LogPath
            ...
            Write-BPERPLog -Message "Configuring database permissions..." -LogPath $script:LogPath
            Set-BPERPDatabasePermissions -Config $config

            # Configure BusinessPlus settings
            Write-BPERPLog -Message "Applying post-restore configurations..." -LogPath $script:LogPath
            Set-BPERPConfiguration -Config $config -TestingMode $TestingMode

            # Copy dashboard files if requested
            if ($RestoreDashboards) {
                Write-BPERPLog -Message "Copying dashboard files..." -LogPath $script:LogPath
                Copy-BPERPDashboardFiles -Config $config
            }

            # Restart servers
            Write-BPERPLog -Message "Restarting BusinessPlus servers..." -LogPath $script:LogPath
            Restart-BPERPServers -Config $config

            # Send completion notification
            $endTime = Get-Date
            Write-BPERPLog -Message "Sending completion notification..." -LogPath $script:LogPath
            Send-BPERPNotification -Config $config -BackupFiles $backupFiles -TestingMode $TestingMode -StartTime $startTime -EndTime $endTime

            Write-BPERPLog -Message "BusinessPlus Database Restore completed successfully!" -LogPath $script:LogPath
            Write-Host "`nBusinessPlus Database Restore completed successfully!" -ForegroundColor Green
        } catch {
            Write-BPERPLog -Message "Error during restore operation: $_" -Level Error -LogPath $script:LogPath
            throw
        }
    }
