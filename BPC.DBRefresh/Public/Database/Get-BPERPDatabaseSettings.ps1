function Get-BPERPDatabaseSettings {
  ...
  #>
    ...
    Write-BPERPLog -Message "Retrieving existing database connection settings" -LogPath $script:LogPath
...
"@
        ...
        $settings = Invoke-DbaQuery -SqlInstance $Config.SQLInstance -Database $Config.SYSCATdb -Query $query

        Write-BPERPLog -Message "Retrieved $($settings.Count) connection settings" -LogPath $script:LogPath
        return $settings
    } catch {
        Write-BPERPLog -Message "Error retrieving database settings: $_" -Level Error -LogPath $script:LogPath
        throw
    }
}
