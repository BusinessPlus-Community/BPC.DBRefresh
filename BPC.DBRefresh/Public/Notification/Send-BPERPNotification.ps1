function Send-BPERPNotification {
  ...
  #>
    ...
    Write-BPERPLog -Message "Preparing email notification" -LogPath $script:LogPath

    try {
        # Load MailKit assemblies
        $mailKitPath = Join-Path $PSScriptRoot -ChildPath ".." | Join-Path -ChildPath ".." | Join-Path -ChildPath ".." | Join-Path -ChildPath ".." | Join-Path -ChildPath "lib" | Join-Path -ChildPath "MailKit"
        if (Test-Path $mailKitPath) {
            Add-Type -Path (Join-Path $mailKitPath "MimeKit.dll")
            Add-Type -Path (Join-Path $mailKitPath "MailKit.dll")
        } else {
            Write-BPERPLog -Message "MailKit assemblies not found, using Send-MailMessage instead" -Level Warning -LogPath $script:LogPath
            ...
            Write-BPERPLog -Message "Email notification sent successfully" -LogPath $script:LogPath
            return
        }
        ...
        Write-BPERPLog -Message "Email notification sent successfully via MailKit" -LogPath $script:LogPath
    } catch {
        Write-BPERPLog -Message "Error sending email notification: $_" -Level Error -LogPath $script:LogPath
        throw
    }
}
